...
namespace toasty::timing::encoding {
    ...
    inline void
    appendRelativeJump(std::vector<uint8_t>& bytes, uintptr_t patchAddress, uintptr_t destination) {
    ...
    inline uint32_t arm64MoveWide(uint32_t base, uint8_t reg, uint16_t immediate, uint8_t shift) {
        return base | (static_cast<uint32_t>(shift / 16) << 21) |
    ...
    inline uint32_t arm64Adrp(uint8_t reg, uintptr_t instruction, uintptr_t target) {
        ...
        return 0x90000000 | (static_cast<uint32_t>(immediate & 3) << 29) |
               (static_cast<uint32_t>((immediate >> 2) & 0x7ffff) << 5) | reg;
    }

    inline uint32_t arm64Load64(uint8_t reg, uint8_t baseReg, uint32_t byteOffset) {
        return 0xf9400000 | ((byteOffset / 8) << 10) | (static_cast<uint32_t>(baseReg) << 5) | reg;
    }

    inline uint32_t arm64LoadFloat(uint8_t reg, uint8_t baseReg, uint32_t byteOffset) {
        return 0xbd400000 | ((byteOffset / 4) << 10) | (static_cast<uint32_t>(baseReg) << 5) | reg;
    }

    inline void
    appendThumbMove(std::vector<uint8_t>& bytes, uint16_t base, uint8_t reg, uint16_t value) {
        auto first =
            static_cast<uint16_t>(base | (((value >> 11) & 1) << 10) | ((value >> 12) & 0xf));
        auto second = static_cast<uint16_t>((((value >> 8) & 7) << 12) |
                                            (static_cast<uint16_t>(reg) << 8) | (value & 0xff));
